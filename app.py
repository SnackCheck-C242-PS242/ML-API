...
def categorize_nutrient(nutrient_name, value, very_low_threshold, low_range, moderate_range, high_range, very_high_min):
    """Categorize a nutrient value based on thresholds."""
    if value < very_low_threshold:
        return f"Very Low in {nutrient_name}"
    elif low_range[0] <= value <= low_range[1]:
        return f"Low in {nutrient_name}"
    elif moderate_range[0] <= value <= moderate_range[1]:
        return f"Moderate in {nutrient_name}"
    elif high_range[0] <= value <= high_range[1]:
        return f"High in {nutrient_name}"
    elif value > very_high_min:
        return f"Very High in {nutrient_name}"
    else:
        return f"Unknown level of {nutrient_name}"
...
def predict_healthiness_tflite(fat, saturated_fat, carbohydrates, sugars, fiber, sodium, proteins):
    ...
    categories = {
        "fat": categorize_nutrient("fat", fat, 2, (2, 4), (4, 6), (6, 8), 8),
        "saturated_fat": categorize_nutrient("saturated fat", saturated_fat, 1, (1, 2), (2, 3), (3, 4), 4),
        "carbohydrates": categorize_nutrient("carbohydrates", carbohydrates, 15, (15, 21), (21, 24), (24, 27), 27),
        "sugars": categorize_nutrient("sugars", sugars, 1, (1, 2), (2, 3), (3, 4), 4),
        "fiber": categorize_nutrient("fiber", fiber, 2, (2, 4), (4, 6), (6, 8), 8),
        "proteins": categorize_nutrient("proteins", proteins, 0, (0, 4), (4, 7), (7, 16), 16),
        "sodium": categorize_nutrient("sodium", sodium, 0.12, (0.12, 0.24), (0.24, 0.36), (0.36, 0.48), 0.48),
    }
